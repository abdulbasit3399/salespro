...
class SaleController extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    if($role->hasPermissionTo('sales-index')) {
      ...
      return view('backend.sale.index', compact('starting_date', 'ending_date', 'warehouse_id', 'sale_status', 'payment_status', 'lims_gift_card_list', 'lims_pos_setting_data', 'lims_reward_point_setting_data', 'lims_account_list', 'lims_warehouse_list', 'all_permission','options', 'numberOfInvoice', 'custom_fields', 'field_name'));
    }
    else
  ...
  public function create()
  {
    ...
    if($role->hasPermissionTo('sales-add')) {
      ...
      return view('backend.sale.create',compact('currency_list', 'lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_tax_list', 'lims_reward_point_setting_data','options', 'numberOfInvoice', 'custom_fields'));
    }
    else
      return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
  }
  ...
  public function store(Request $request)
  {
    $data = $request->all();
        ...
        /*DB::beginTransaction();
        try {*/
          if($data['paid_by_id'] == 8)
          {
            $load_card = LoadCard::find($data['load_card_id']);
            $pos_set = PosSetting::first();

            $balance = $load_card->amount - $load_card->expense + abs($pos_set->load_card_negative_balance);
            if($balance < $data['paid_amount'])
              return redirect()->back()->with('message', 'Amount exceeds card balance!');
          }

          if($data['paid_by_id'] == 2)
          {
            $load_card = GiftCard::find($data['gift_card_id']);

            $balance = $load_card->amount - $load_card->expense;
            if($balance < $data['paid_amount'])
              return redirect()->back()->with('message', 'Amount exceeds card balance!');
          }
    ...
    public function edit($id)
    {
      ...
      if($role->hasPermissionTo('sales-edit')){
        ...
        return view('backend.sale.edit',compact('lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_tax_list', 'lims_sale_data','lims_product_sale_data', 'currency_exchange_rate'));
      }
      else
        return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function genInvoice($id)
    {
      ...
      if($lims_pos_setting_data->invoice_option == 'A4') {
        return view('backend.sale.a4_invoice', compact('lims_sale_data', 'currency_code', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords', 'paid_by_info', 'sale_custom_fields', 'customer_custom_fields'));
      }
      else{
        return view('backend.sale.invoice', compact('lims_sale_data', 'currency_code', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords', 'sale_custom_fields', 'customer_custom_fields'));
      }
    }
    ...
    public function fast_recharge_load_card()
    {
      $load_cards = LoadCard::where('is_active', true)->orderBy('id', 'desc')->get();
      return view('backend.sale.fast_recharge',compact('load_cards'));
    }
    public function recharge_load_card_save(Request $request)
    {
      $this->validate($request,[
        'load_card_id' => 'required',
        'amount' => 'required|numeric|min:1',
      ]);
      $card = LoadCard::find($request->load_card_id);
      $card->amount += $request->amount;
      $card->save();

      $recharge = new LoadCardRecharge;
      $recharge->user_id = Auth::id();
      $recharge->load_card_id = $request->load_card_id;
      $recharge->amount = $request->amount;
      $recharge->save();


      return redirect()->back()->with('message', 'Recharge successfully.');
    }
